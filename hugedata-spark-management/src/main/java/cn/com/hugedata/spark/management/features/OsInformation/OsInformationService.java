package cn.com.hugedata.spark.management.features.OsInformation;
...
public class OsInformationService 
        extends FeatureServiceImpl<PtArticle, Integer, OsInformationModel, PtArticleMapper> {

    private static final Logger LOGGER = LoggerFactory.getLogger(OsInformationService.class);
    ...
    public PageEntity<PtArticle> list(Map<String, Object> query, int pageStart, int pageSize, List<OrderItem> orders) {
        query.put(PtArticle.Query.CATEGORY_ID__IN,
                Arrays.asList(PtCategory.INFORMATION_COMPANIES,
                              PtCategory.INFORMATION_PRODUCTS));
    ...
    @Override
    public PtArticle add(OsInformationModel model) throws ServiceException {
        parseDate(model);
    ...
    @Override
    public void modify(OsInformationModel model) throws ServiceException {
        parseDate(model);
        model.setUpdateTime(new Date());
        super.modify(model);
    ...
    @Override
    public OsInformationModel queryForModify(Integer id) throws ServiceException {
        OsInformationModel model = super.queryForModify(id);
     ...
     * 处理Model中的日期.
     */
    private void parseDate(OsInformationModel model) throws ServiceException {
        try {
     ...
     * 保存图片信息.
     */
    private void saveImages(OsInformationModel model, int articleId, String categoryId) throws ServiceException {
     ...
     * 保存附件信息.
     */
    private void saveAttachments(OsInformationModel model, int articleId, String categoryId) {
     ...
     * 加载图片信息到Model.
     */
    private void loadImagesForModify(OsInformationModel model, Integer id) {
     ...
     * 加载附件信息到Model.
     */
    private void loadAttachmentsForModify(OsInformationModel model, Integer id) {
