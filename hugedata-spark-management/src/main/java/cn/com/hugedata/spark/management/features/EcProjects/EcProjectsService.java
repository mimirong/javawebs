package cn.com.hugedata.spark.management.features.EcProjects;
...
@Service
public class EcProjectsService
        extends FeatureServiceImpl<PtArticle, Integer, EcProjectsModel, PtArticleMapper> {

    private static final Logger LOGGER = LoggerFactory.getLogger(EcProjectsService.class);
    ...
    public PageEntity<PtArticle> list(Map<String, Object> query, int pageStart, int pageSize, List<OrderItem> orders) {
        query.put(PtArticle.Query.CATEGORY_ID__IN, Arrays.asList(PtCategory.ECOMM_PROJECTS));
    ...
    @Override
    public PtArticle add(EcProjectsModel model) throws ServiceException {
        parseDate(model);
    ...
    @Override
    public void modify(EcProjectsModel model) throws ServiceException {
        parseDate(model);
        model.setUpdateTime(new Date());
        super.modify(model);
    ...
    @Override
    public EcProjectsModel queryForModify(Integer id) throws ServiceException {
        EcProjectsModel model = super.queryForModify(id);
     ...
     * 处理Model中的日期.
     */
    private void parseDate(EcProjectsModel model) throws ServiceException {
        try {
     ...
     * 保存图片信息.
     */
    private void saveImages(EcProjectsModel model, int articleId, String categoryId) throws ServiceException {
     ...
     * 保存附件信息.
     */
    private void saveAttachments(EcProjectsModel model, int articleId, String categoryId) {
     ...
     * 加载图片信息到Model.
     */
    private void loadImagesForModify(EcProjectsModel model, Integer id) {
     ...
     * 加载附件信息到Model.
     */
    private void loadAttachmentsForModify(EcProjectsModel model, Integer id) {
