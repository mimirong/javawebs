package cn.com.hugedata.spark.management.features.GaSiteService;
...
@Service
public class GaSiteServiceService
        extends FeatureServiceImpl<PtArticle, Integer,  GaSiteServiceModel, PtArticleMapper> {

    private static final Logger LOGGER = LoggerFactory.getLogger(GaSiteServiceService.class);
    ...
    public PageEntity<PtArticle> list(Map<String, Object> query, int pageStart, int pageSize, List<OrderItem> orders) {
        query.put(PtArticle.Query.CATEGORY_ID__IN, Arrays.asList(PtCategory.GA_SITE));
    ...
    @Override
    public PtArticle add(GaSiteServiceModel model) throws ServiceException {
        parseDate(model);
    ...
    @Override
    public void modify(GaSiteServiceModel model) throws ServiceException {
        parseDate(model);
        model.setUpdateTime(new Date());
        super.modify(model);
    ...
    @Override
    public GaSiteServiceModel queryForModify(Integer id) throws ServiceException {
        GaSiteServiceModel model = super.queryForModify(id);
     ...
     * 处理Model中的日期.
     */
    private void parseDate(GaSiteServiceModel model) throws ServiceException {
        try {
     ...
     * 保存图片信息.
     */
    private void saveImages(GaSiteServiceModel model, int articleId, String categoryId) throws ServiceException {
     ...
     * 保存附件信息.
     */
    private void saveAttachments(GaSiteServiceModel model, int articleId, String categoryId) {
     ...
     * 加载图片信息到Model.
     */
    private void loadImagesForModify(GaSiteServiceModel model, Integer id) {
     ...
     * 加载附件信息到Model.
     */
    private void loadAttachmentsForModify(GaSiteServiceModel model, Integer id) {
