...
@Service
public class GaFeeServiceService
        extends FeatureServiceImpl<PtArticle, Integer, GaFeeServiceModel, PtArticleMapper>  {

    private static final Logger LOGGER = LoggerFactory.getLogger(GaFeeServiceService.class);
    ...
    public PageEntity<PtArticle> list(Map<String, Object> query, int pageStart, int pageSize, List<OrderItem> orders) {
        query.put(PtArticle.Query.CATEGORY_ID__IN, Arrays.asList(PtCategory.GA_FEES));
    ...
    @Override
    public PtArticle add(GaFeeServiceModel model) throws ServiceException {
        parseDate(model);
    ...
    @Override
    public void modify(GaFeeServiceModel model) throws ServiceException {
        parseDate(model);
        model.setUpdateTime(new Date());
        super.modify(model);
    ...
    @Override
    public GaFeeServiceModel queryForModify(Integer id) throws ServiceException {
        GaFeeServiceModel model = super.queryForModify(id);
     ...
     * 处理Model中的日期.
     */
    private void parseDate(GaFeeServiceModel model) throws ServiceException {
        try {
     ...
     * 保存图片信息.
     */
    private void saveImages(GaFeeServiceModel model, int articleId, String categoryId) throws ServiceException {
     ...
     * 保存附件信息.
     */
    private void saveAttachments(GaFeeServiceModel model, int articleId, String categoryId) {
     ...
     * 加载图片信息到Model.
     */
    private void loadImagesForModify(GaFeeServiceModel model, Integer id) {
     ...
     * 加载附件信息到Model.
     */
    private void loadAttachmentsForModify(GaFeeServiceModel model, Integer id) {
