...
@Service
public class OsHumanResourceService
        extends FeatureServiceImpl<PtArticle, Integer, OsHumanResourceModel, PtArticleMapper> {

    private static final Logger LOGGER = LoggerFactory.getLogger(OsHumanResourceService.class);
    ...
    public PageEntity<PtArticle> list(Map<String, Object> query, int pageStart, int pageSize, List<OrderItem> orders) {
        query.put(PtArticle.Query.CATEGORY_ID__IN,
                Arrays.asList(PtCategory.HUMAN_RESOURCE_1,
                              PtCategory.HUMAN_RESOURCE_2,
                              PtCategory.HUMAN_RESOURCE_3));
    ...
    @Override
    public PtArticle add(OsHumanResourceModel model) throws ServiceException {
        parseDate(model);
    ...
    @Override
    public void modify(OsHumanResourceModel model) throws ServiceException {
        parseDate(model);
        model.setUpdateTime(new Date());
        super.modify(model);
    ...
    @Override
    public OsHumanResourceModel queryForModify(Integer id) throws ServiceException {
        OsHumanResourceModel model = super.queryForModify(id);
     ...
     * 处理Model中的日期.
     */
    private void parseDate(OsHumanResourceModel model) throws ServiceException {
        try {
     ...
     * 保存图片信息.
     */
    private void saveImages(OsHumanResourceModel model, int articleId, String categoryId) throws ServiceException {
     ...
     * 保存附件信息.
     */
    private void saveAttachments(OsHumanResourceModel model, int articleId, String categoryId) {
     ...
     * 加载图片信息到Model.
     */
    private void loadImagesForModify(OsHumanResourceModel model, Integer id) {
     ...
     * 加载附件信息到Model.
     */
    private void loadAttachmentsForModify(OsHumanResourceModel model, Integer id) {
