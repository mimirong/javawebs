...
public class LoginService {
     ...
     * @param loginName         用户名
     * @param password          密码
     * @param expectedUserType  允许登录的用户类型
     * @param session           Http Session
     * @return                  登录成功返回token和用户信息
     * @throws ServiceException 登录失败，异常包含错误信息
     */
    public LoginResult login(String loginName, String password, String expectedUserType, HttpSession session) throws ServiceException {
        // 检查参数
        ...
        // 检查用户类型
        if (user.getUserType().equalsIgnoreCase(UcUserInfo.USER_TYPE_SYSTEM)) {
            if (expectedUserType.equalsIgnoreCase(UcUserInfo.USER_TYPE_PARK)) {
                LOGGER.error("Invalid user type to login: {}", user.getUserType());
                throw new ServiceException("InvalidUserType", "请以管理员登录");
            }
        } else if (user.getUserType().equalsIgnoreCase(UcUserInfo.USER_TYPE_PARK)) {
            if (expectedUserType.equalsIgnoreCase(UcUserInfo.USER_TYPE_SYSTEM)) {
                LOGGER.error("Invalid user type to login: {}", user.getUserType());
                throw new ServiceException("InvalidUserType", "请以普通用户登录");
            }
        } else {
            throw new ServiceException("InvalidUserType", "用户名或密码错误");
        }
        
        // 登录检查成功
        LoginSessionInfo loginInfo = new LoginSessionInfo();
        loginInfo.setUserInfo(user);
        session.setAttribute(ServerConstants.LOGIN_INFO_SESSION_KEY, loginInfo);
    ...
    public void logout(HttpSession session) {
        session.removeAttribute(ServerConstants.LOGIN_INFO_SESSION_KEY);
    }
    ...
    private void checkPassword(UcUserInfo user, String password) throws ServiceException {
        ...
        throw new ServiceException("LoginService-WrongPassword", "用户名或密码错误");
    }
