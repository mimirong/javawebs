...
@Service
public class OsIntellectualService 
        extends FeatureServiceImpl<PtArticle, Integer, OsIntellectualModel, PtArticleMapper> {

    private static final Logger LOGGER = LoggerFactory.getLogger(OsIntellectualService.class);
    ...
    public PageEntity<PtArticle> list(Map<String, Object> query, int pageStart, int pageSize, List<OrderItem> orders) {
        query.put(PtArticle.Query.CATEGORY_ID__IN,
                Arrays.asList(PtCategory.INTELLECTUAL_1,
                              PtCategory.INTELLECTUAL_2));
    ...
    @Override
    public PtArticle add(OsIntellectualModel model) throws ServiceException {
        parseDate(model);
    ...
    @Override
    public void modify(OsIntellectualModel model) throws ServiceException {
        parseDate(model);
        model.setUpdateTime(new Date());
        super.modify(model);
    ...
    @Override
    public OsIntellectualModel queryForModify(Integer id) throws ServiceException {
        OsIntellectualModel model = super.queryForModify(id);
     ...
     * 处理Model中的日期.
     */
    private void parseDate(OsIntellectualModel model) throws ServiceException {
        try {
     ...
     * 保存图片信息.
     */
    private void saveImages(OsIntellectualModel model, int articleId, String categoryId) throws ServiceException {
     ...
     * 保存附件信息.
     */
    private void saveAttachments(OsIntellectualModel model, int articleId, String categoryId) {
     ...
     * 加载图片信息到Model.
     */
    private void loadImagesForModify(OsIntellectualModel model, Integer id) {
     ...
     * 加载附件信息到Model.
     */
    private void loadAttachmentsForModify(OsIntellectualModel model, Integer id) {
