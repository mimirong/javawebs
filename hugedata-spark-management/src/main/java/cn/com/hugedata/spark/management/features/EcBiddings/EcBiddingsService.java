package cn.com.hugedata.spark.management.features.EcBiddings;
...
@Service
public class EcBiddingsService 
        extends FeatureServiceImpl<PtArticle, Integer, EcBiddingsModel, PtArticleMapper> {

    private static final Logger LOGGER = LoggerFactory.getLogger(EcBiddingsService.class);
    ...
    public PageEntity<PtArticle> list(Map<String, Object> query, int pageStart, int pageSize, List<OrderItem> orders) {
        query.put(PtArticle.Query.CATEGORY_ID__IN, Arrays.asList(PtCategory.ECOMM_BIDDINGS));
    ...
    @Override
    public PtArticle add(EcBiddingsModel model) throws ServiceException {
        parseDate(model);
    ...
    @Override
    public void modify(EcBiddingsModel model) throws ServiceException {
        parseDate(model);
        model.setUpdateTime(new Date());
        super.modify(model);
    ...
    @Override
    public EcBiddingsModel queryForModify(Integer id) throws ServiceException {
        EcBiddingsModel model = super.queryForModify(id);
     ...
     * 处理Model中的日期.
     */
    private void parseDate(EcBiddingsModel model) throws ServiceException {
        try {
     ...
     * 保存图片信息.
     */
    private void saveImages(EcBiddingsModel model, int articleId, String categoryId) throws ServiceException {
     ...
     * 保存附件信息.
     */
    private void saveAttachments(EcBiddingsModel model, int articleId, String categoryId) {
     ...
     * 加载图片信息到Model.
     */
    private void loadImagesForModify(EcBiddingsModel model, Integer id) {
     ...
     * 加载附件信息到Model.
     */
    private void loadAttachmentsForModify(EcBiddingsModel model, Integer id) {
