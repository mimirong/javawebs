...
@Service
public class EcFinancingService 
        extends FeatureServiceImpl<PtArticle, Integer, EcFinancingModel, PtArticleMapper> {

    private static final Logger LOGGER = LoggerFactory.getLogger(EcFinancingService.class);
    ...
    public PageEntity<PtArticle> list(Map<String, Object> query, int pageStart, int pageSize, List<OrderItem> orders) {
        query.put(PtArticle.Query.CATEGORY_ID__IN, Arrays.asList(PtCategory.ECOMM_FINANCING));
    ...
    @Override
    public PtArticle add(EcFinancingModel model) throws ServiceException {
        parseDate(model);
    ...
    @Override
    public void modify(EcFinancingModel model) throws ServiceException {
        parseDate(model);
        model.setUpdateTime(new Date());
        super.modify(model);
    ...
    @Override
    public EcFinancingModel queryForModify(Integer id) throws ServiceException {
        EcFinancingModel model = super.queryForModify(id);
     ...
     * 处理Model中的日期.
     */
    private void parseDate(EcFinancingModel model) throws ServiceException {
        try {
     ...
     * 保存图片信息.
     */
    private void saveImages(EcFinancingModel model, int articleId, String categoryId) throws ServiceException {
     ...
     * 保存附件信息.
     */
    private void saveAttachments(EcFinancingModel model, int articleId, String categoryId) {
     ...
     * 加载图片信息到Model.
     */
    private void loadImagesForModify(EcFinancingModel model, Integer id) {
     ...
     * 加载附件信息到Model.
     */
    private void loadAttachmentsForModify(EcFinancingModel model, Integer id) {
