...
@Service
public class EcOutsourcingService 
        extends FeatureServiceImpl<PtArticle, Integer, EcOutsourcingModel, PtArticleMapper> {

    private static final Logger LOGGER = LoggerFactory.getLogger(EcOutsourcingService.class);
    ...
    public PageEntity<PtArticle> list(Map<String, Object> query, int pageStart, int pageSize, List<OrderItem> orders) {
        query.put(PtArticle.Query.CATEGORY_ID__IN, Arrays.asList(PtCategory.ECOMM_OUTSOURCING));
    ...
    @Override
    public PtArticle add(EcOutsourcingModel model) throws ServiceException {
        parseDate(model);
    ...
    @Override
    public void modify(EcOutsourcingModel model) throws ServiceException {
        parseDate(model);
        model.setUpdateTime(new Date());
        super.modify(model);
    ...
    @Override
    public EcOutsourcingModel queryForModify(Integer id) throws ServiceException {
        EcOutsourcingModel model = super.queryForModify(id);
     ...
     * 处理Model中的日期.
     */
    private void parseDate(EcOutsourcingModel model) throws ServiceException {
        try {
     ...
     * 保存图片信息.
     */
    private void saveImages(EcOutsourcingModel model, int articleId, String categoryId) throws ServiceException {
     ...
     * 保存附件信息.
     */
    private void saveAttachments(EcOutsourcingModel model, int articleId, String categoryId) {
     ...
     * 加载图片信息到Model.
     */
    private void loadImagesForModify(EcOutsourcingModel model, Integer id) {
     ...
     * 加载附件信息到Model.
     */
    private void loadAttachmentsForModify(EcOutsourcingModel model, Integer id) {
