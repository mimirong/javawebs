package cn.com.hugedata.spark.management.features.OsInvest;
...
@Service
public class OsInvestService 
        extends FeatureServiceImpl<PtArticle, Integer, OsInvestModel, PtArticleMapper> {

    private static final Logger LOGGER = LoggerFactory.getLogger(OsInvestService.class);
    ...
    public PageEntity<PtArticle> list(Map<String, Object> query, int pageStart, int pageSize, List<OrderItem> orders) {
        query.put(PtArticle.Query.CATEGORY_ID__IN,
                Arrays.asList(PtCategory.OSINVEST_1,
                              PtCategory.OSINVEST_2));
    ...
    @Override
    public PtArticle add(OsInvestModel model) throws ServiceException {
        parseDate(model);
    ...
    @Override
    public void modify(OsInvestModel model) throws ServiceException {
        parseDate(model);
        model.setUpdateTime(new Date());
        super.modify(model);
    ...
    @Override
    public OsInvestModel queryForModify(Integer id) throws ServiceException {
        OsInvestModel model = super.queryForModify(id);
     ...
     * 处理Model中的日期.
     */
    private void parseDate(OsInvestModel model) throws ServiceException {
        try {
     ...
     * 保存图片信息.
     */
    private void saveImages(OsInvestModel model, int articleId, String categoryId) throws ServiceException {
     ...
     * 保存附件信息.
     */
    private void saveAttachments(OsInvestModel model, int articleId, String categoryId) {
     ...
     * 加载图片信息到Model.
     */
    private void loadImagesForModify(OsInvestModel model, Integer id) {
     ...
     * 加载附件信息到Model.
     */
    private void loadAttachmentsForModify(OsInvestModel model, Integer id) {
