      ...
      <div class="tab-content tab-content-right">
          ...
          <table class="table mt20" style="height: 96px;">
              <tr>
                  ...
                  <td colspan="3">
                      <textarea name="" id="approveInfo" cols="30" rows="10">${report.approveInfo }</textarea>
                  </td>
              </tr>
          </table>
          <div style="margin: 20px 0;text-align: center;"><a ng-click="submitReport(${report.reportId })" class="btn">提交</a></div>
      </div>
...
if($('#investedNum').val() != ''){
	$('#investedNum').val(parseFloat($('#investedNum').val()).toFixed(2));	
}
if($('#amount').text() != '暂无数据'){
	...
	if($('#amount').text() != '0.00'){
		...
		$('#useDesign').val(parseFloat($('#useDesign').val()).toFixed(2));
		$('#useOversee').val(parseFloat($('#useOversee').val()).toFixed(2));
		$('#useEngineer').val(parseFloat($('#useEngineer').val()).toFixed(2));
		$('#useBuy').val(parseFloat($('#useBuy').val()).toFixed(2));
		$('#useAsset').val(parseFloat($('#useAsset').val()).toFixed(2));
		$('#useTotal').val(parseFloat($('#useTotal').val()).toFixed(2));
