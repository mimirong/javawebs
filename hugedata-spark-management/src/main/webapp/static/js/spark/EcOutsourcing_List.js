...
var columns =[
	{
		...
		render: function(val) {
			switch (val) {
				case "ECOMM_OUTSOURCING": 	return "服务外包";
				default:				return val;
			}
		}
	}, {
		...
		render: function(row) {
			var m = [];
			if (CommonsPrivileges.hasPrivilegeId("ecomm/outsourcing/setVisible")) {
				if (row.isVisible) {
			...
			if (CommonsPrivileges.hasPrivilegeId("ecomm/outsourcing/modify")) {
				m.push('<a class="txt-op btnModify" data-row-id="' + row.articleId + '" href="javascript:;">修改</a>');
			}
			if (CommonsPrivileges.hasPrivilegeId("ecomm/outsourcing/delete")) {
