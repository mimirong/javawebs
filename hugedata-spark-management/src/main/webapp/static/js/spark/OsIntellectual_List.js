...
var columns =[
	{
		...
		render: function(val) {
			switch (val) {
				case "INTELLECTUAL_1": 	return "办事服务";
				case "INTELLECTUAL_2": 	return "数据服务";
				default:				return val;
			}
		}
	}, {
		...
		render: function(row) {
			var m = [];
			if (CommonsPrivileges.hasPrivilegeId("outtech/intellectual/setVisible")) {
				if (row.isVisible) {
			...
			if (CommonsPrivileges.hasPrivilegeId("outtech/intellectual/modify")) {
				m.push('<a class="txt-op btnModify" data-row-id="' + row.articleId + '" href="javascript:;">修改</a>');
			}
			if (CommonsPrivileges.hasPrivilegeId("outtech/intellectual/delete")) {
