...
var columns =[
	{
		...
		render: function(val) {
			switch (val) {
				case "OSINVEST_1": 	return "招商信息";
				case "OSINVEST_2": 	return "资源展示";
				default:			return val;
			}
		}
	}, {
		...
		render: function(row) {
			var m = [];
			if (CommonsPrivileges.hasPrivilegeId("outtech/invest/setVisible")) {
				if (row.isVisible) {
			...
			if (CommonsPrivileges.hasPrivilegeId("outtech/invest/modify")) {
				m.push('<a class="txt-op btnModify" data-row-id="' + row.articleId + '" href="javascript:;">修改</a>');
			}
			if (CommonsPrivileges.hasPrivilegeId("outtech/invest/delete")) {
