...
var columns =[
	{
		...
		render: function(row) {
			var m = [];
			if (CommonsPrivileges.hasPrivilegeId("joinguide/modify")) {
				m.push('<a class="txt-op btnModify" data-row-id="' + row.guideId + '" href="javascript:;">修改</a>');
			}
			if (CommonsPrivileges.hasPrivilegeId("joinguide/delete")) {
...
dt.on("draw", function(settings, data) {
	...
	$(".btnModify").off().on("click", function(e) {
		...
		$.ajax({
			...
			error: function() {
				MU.msgTips("error", "获取入园指南信息失败，请稍后重试", 1200);
			}
		});
	});
});
...
function checkAddModifyForm() {
	...
	var title = $("#title").val();
	if (title == "") {
		MU.msgTips("warn", "请输入入园指南名称", 1200);
		$("#title").focus();
		return null;
	}
