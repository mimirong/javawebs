...
var columns =[
	{
		...
		render: function(row) {
			var m = [];
			if (CommonsPrivileges.hasPrivilegeId("fees/setVisible")) {
				if (row.isVisible) {
			...
			if (CommonsPrivileges.hasPrivilegeId("fees/modify")) {
				m.push('<a class="txt-op btnModify" data-row-id="' + row.articleId + '" href="javascript:;">修改</a>');
			}
			if (CommonsPrivileges.hasPrivilegeId("fees/delete")) {
