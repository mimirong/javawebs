...
var columns =[
    {
        ...
        render: function(val) {
            switch (val) {
                case "HANDY_SERVICE_RESIDENCE": 	return "园区小区";
                case "HANDY_SERVICE_SCHOOL": 	    return "园区学校";
                case "HANDY_SERVICE_HOSPITAL": 	    return "园区医院";
                case "HANDY_SERVICE_BUS": 	        return "园区公交";
                case "HANDY_SERVICE_BANK":          return "园区银行";
                default:					        return val;
            }
        }
    }, {
        ...
        render: function(row) {
            var m = [];
            if (CommonsPrivileges.hasPrivilegeId("handy_service/preview")) {
                m.push('<a href=' + govAffairsPortalUrl + '/handyService/details?articleId=' + row.articleId + '&categoryId=' + row.categoryId + '&isHideBack=1  target ="_black" class="txt-op" data-row-id="' + row.articleId + '">预览</a> ');
            }
            if (CommonsPrivileges.hasPrivilegeId("handy_service/setVisible")) {
                if (row.isVisible) {
            ...
            if (CommonsPrivileges.hasPrivilegeId("handy_service/modify")) {
                m.push('<a class="txt-op btnModify" data-row-id="' + row.articleId + '" href="javascript:;">修改</a>');
            }
            if (CommonsPrivileges.hasPrivilegeId("handy_service/delete")) {
...
// 栏目查询选择
$(function() {
	$("#categoryIdQuerySelect").siblings("ul").find("li").on("click", function() {
		var value = $(this).attr("data-value");
		$("#categoryIdQuery").val(value);
		dt.ajax.reload();
	});
});
...
$(".btnAdd").on("click", function() {
    ...
    $("#categoryId").val($("#categoryIdQuery").val()).triggerHandler("change");
