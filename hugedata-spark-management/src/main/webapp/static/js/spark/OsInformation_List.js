...
var columns =[
	{
		...
		render: function(val) {
			switch (val) {
				case "INFORMATION_COMPANIES":return "企业信息库";
				case "INFORMATION_PRODUCTS": return "产品信息库";
				default:					 return val;
			}
		}
	}, {
		...
		render: function(row) {
			var m = [];
			if (CommonsPrivileges.hasPrivilegeId("outtech/information/setVisible")) {
				if (row.isVisible) {
			...
			if (CommonsPrivileges.hasPrivilegeId("outtech/information/modify")) {
				m.push('<a class="txt-op btnModify" data-row-id="' + row.articleId + '" href="javascript:;">修改</a>');
			}
			if (CommonsPrivileges.hasPrivilegeId("outtech/information/delete")) {
