...
var columns =[
	{
		...
		render: function(val) {
			switch (val) {
				case "ECOMM_PROJECTS": 	return "项目对接";
				default:				return val;
			}
		}
	}, {
		...
		render: function(row) {
			var m = [];
			if (CommonsPrivileges.hasPrivilegeId("ecomm/projects/setVisible")) {
				if (row.isVisible) {
			...
			if (CommonsPrivileges.hasPrivilegeId("ecomm/projects/modify")) {
				m.push('<a class="txt-op btnModify" data-row-id="' + row.articleId + '" href="javascript:;">修改</a>');
			}
			if (CommonsPrivileges.hasPrivilegeId("ecomm/projects/delete")) {
