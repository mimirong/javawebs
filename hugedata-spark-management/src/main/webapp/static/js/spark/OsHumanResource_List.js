...
var columns =[
	{
		...
		render: function(val) {
			switch (val) {
				case "HUMAN_RESOURCE_1": 	return "人才培训";
				case "HUMAN_RESOURCE_2": 	return "技术职称招考";
				case "HUMAN_RESOURCE_3": 	return "企业招聘";
				default:					return val;
			}
		}
	}, {
		...
		render: function(row) {
			var m = [];
			if (CommonsPrivileges.hasPrivilegeId("outtech/hr/setVisible")) {
				if (row.isVisible) {
			...
			if (CommonsPrivileges.hasPrivilegeId("outtech/hr/modify")) {
				m.push('<a class="txt-op btnModify" data-row-id="' + row.articleId + '" href="javascript:;">修改</a>');
			}
			if (CommonsPrivileges.hasPrivilegeId("outtech/hr/delete")) {
