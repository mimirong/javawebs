...
var columns =[
    {
        ...
        render: function(val) {
            switch (val) {
                case "NOTICE_PICTURE_NEWS": 	return "图片新闻";
                case "NOTICE_NOTICE": 	        return "通知公告";
                default:					    return val;
            }
        }
    }, {
        ...
        render: function(row) {
            var m = [];
            if(CommonsPrivileges.hasPrivilegeId("notice/preview")){
                m.push('<a href=' + govAffairsPortalUrl + '/notice/details?articleId=' + row.articleId + '&categoryId=' + row.categoryId + '  target ="_black" class="txt-op" data-row-id="' + row.articleId + '">预览</a> ');
            }
            if (CommonsPrivileges.hasPrivilegeId("notice/setVisible")) {
                if (row.isVisible) {
            ...
            if (CommonsPrivileges.hasPrivilegeId("notice/modify")) {
                m.push('<a class="txt-op btnModify" data-row-id="' + row.articleId + '" href="javascript:;">修改</a>');
            }
            if (CommonsPrivileges.hasPrivilegeId("notice/delete")) {
