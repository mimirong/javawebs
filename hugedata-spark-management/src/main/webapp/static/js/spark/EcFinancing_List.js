...
var columns =[
	{
		...
		render: function(val) {
			switch (val) {
				case "ECOMM_FINANCING": return "金融服务";
				default:				return val;
			}
		}
	}, {
		...
		render: function(row) {
			var m = [];
			if (CommonsPrivileges.hasPrivilegeId("ecomm/financing/setVisible")) {
				if (row.isVisible) {
			...
			if (CommonsPrivileges.hasPrivilegeId("ecomm/financing/modify")) {
				m.push('<a class="txt-op btnModify" data-row-id="' + row.articleId + '" href="javascript:;">修改</a>');
			}
			if (CommonsPrivileges.hasPrivilegeId("ecomm/financing/delete")) {
