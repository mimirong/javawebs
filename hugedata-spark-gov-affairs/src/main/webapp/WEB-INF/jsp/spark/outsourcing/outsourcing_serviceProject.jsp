...
<div class="cen ng-cloak" id="minServiceH" ng-app="outsourcingServiceProjectModule" ng-controller="outsourcingServiceProjectController">
    <h1 class="column_tit">您当前的位置：<a href="${contextPath}/outsourcing/">首页</a> &gt; <span>{{supplyTypeName}}</span></h1>
    <div class="se-title-tab clearfix">
        <div class="fl">
            <input id="supplyTypeQuery" value="SP_SUPPLY_TYPE_INSPECT_DETEC" type="hidden" name="supplyType">
            <a id="t1" class="on" ng-click="supplyTypeSelected($event, 'SP_SUPPLY_TYPE_INSPECT_DETEC');">检验检测</a><a id="t2" ng-click="supplyTypeSelected($event, 'SP_SUPPLY_TYPE_TECH_SERVICE');">技术服务</a>
        </div>
        <div class="fr"><a href="${contextPath}/outsourcing/myRelease" class="btn">我的发布</a><span>信息共 <b>{{page.recordsTotal}}</b> 条</span></div>
    </div>
...
<script>
    ...
    if(supplyType && supplyType == 'SP_SUPPLY_TYPE_TECH_SERVICE'){
    	window.supplyType = supplyType;
    	 $('#t1').removeClass('on');
         $('#t2').addClass('on');
    }else{
...
<script src="${contextPath}/static/js/outsourcing/outsourcing_serviceProject.js?v=${buildTimestamp}"></script>
</body>
</html>